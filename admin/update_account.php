<?php
	...
	elseif($cpass!=$srow['password']){
		...
		mysqli_query($conn,"insert into activitylog (userid,action,activity_date) values ('".$_SESSION['id']."','Cuenta actualizada',NOW())");
		?>
		<script>
			window.alert('Cuenta actualizada exitosamente!');
			window.history.back();
		</script>
		<?php
	}
