<?php
	...
	if (empty($_FILES["image"]["name"])){
		...
		if ($fileInfo['extension'] == "jpg" OR $fileInfo['extension'] == "png") {
			...
			?>
				<script>
					window.alert('Foto no actualizada ¡Sube solo una foto JPG o PNG');
				</script>
			<?php
		}
	}
	...
	if($qty!=$prow['product_qty']){
		mysqli_query($conn,"insert into inventory (userid,action,productid,quantity,inventory_date) values ('".$_SESSION['id']."','Actualizada', '$id', '$qty', NOW())");
	}
	?>
		<script>
